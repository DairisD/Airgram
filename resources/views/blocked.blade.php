...
<div class="container mt-3 w-50" style="background-color: #eeeeee">
    <div class="row pt-2 pb-2">
        ...
        <div class="col-8 d-flex flex-wrap align-items-center justify-content-center">
            <a href="/profile/{{$user->id}}"><button type="button" class="btn btn-outline-dark w-40 mr-1">View Profile</button></a>
            <form method="post" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <input name='id' id='id' value='{{$user->id}}' type='hidden'>
                <button  type="submit" class="btn btn-outline-danger w-40 ml-1">Unblock</button>
            </form>
        </div>
    </div>
</div>
