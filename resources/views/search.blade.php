    ...
    <div class="container bg-light mt-2">
        ...
        <div class="row p-2" style="background-color: #ebebeb">
            <div class="col d-flex align-items-center justify-content-center">
                <form action="/votepos/{{ $user->id }}/{{ $result->image_id }}" method="POST">
                    @csrf
                    ...
                    @if($result->summa)
                    <div class="d-flex align-items-center p-2" style="margin-bottom: 0px; font-size: 20px;">{{ $result->summa }}</div>
                    @else
                    <div class="d-flex align-items-center p-2" style="margin-bottom: 0px; font-size: 20px;">0</div>
                    @endif
                    <form action="/voteneg/{{ $user->id }}/{{ $result->image_id }}" method="POST">
                    @csrf
                    <button type="submit" class="d-flex align-items-center" style="border: none; background-color: #ebebeb;">
            ...
            <div class="col d-flex justify-content-center">
                <a href="/comments/{{ $result->image_id }}" class="d-flex align-items-center p-2 justify-content-center" style="border: none; background-color: #ebebeb;">
    ...
    <div class="container mt-3 w-50" style="background-color: #eeeeee">
        <div class="row pt-2 pb-2">
            <div class="col-4 d-flex justify-content-center align-items-center flex-wrap" style="border-right: 1px solid #969696">
                <img src={{ $users->profile_picture }} style="border-radius: 100%; max-height:50px;">
                <h5 class="ml-2 mb-0">{{ $users->username }}</h5>
            </div>
            <div class="col-8 d-flex flex-wrap align-items-center justify-content-center">
                <a href="/profile/{{$users->id}}"><button type="button" class="btn btn-outline-dark w-40 mr-1">View Profile</button></a>
            </div>
        </div>
    </div>
    @endempty
